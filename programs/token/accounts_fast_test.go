...
// TestSPLLayoutSizes confirms the Go struct sizes exactly match the
// SPL wire sizes. If Go ever inserts padding (compiler upgrade, field
// reorder, ...) this test plus the init()-time AssertPOD call catch
...
// TestAssertPODPasses is redundant with init() (which panics on
// failure) but makes the contract visible in test output.
...
// helper: build an SPL-Token Mint account payload via the traditional
// encoder. Used as the source of truth for parity tests.
...
func TestDecodeMintFast_ParityWithTraditional_AllSet(t *testing.T) {
	...
	// Traditional decode.
	var tradDecoded Mint
	if err := bin.NewBinDecoder(wire).Decode(&tradDecoded); err != nil {
		t.Fatal(err)
	}

	// Fast decode.
	fastDecoded, err := DecodeMintFast(wire)
	if err != nil {
		t.Fatal(err)
	}

	assertMintEqual(t, &tradDecoded, &fastDecoded)
}
...
func TestDecodeAccountFast_ParityWithTraditional_AllSet(t *testing.T) {
	...
	native := uint64(2_039_280) // typical wrapped-SOL rent-exempt reserve
	orig := &Account{
		Mint:            mint,
		Owner:           owner,
		Amount:          999_999,
...
func TestDecodeAccountFast_ParityWithTraditional_NoOptionals(t *testing.T) {
	...
	orig := &Account{
		...
		Amount: 1,
		State:  Initialized,
		// Delegate / IsNative / CloseAuthority absent.
	}
	wire := buildAccountWire(t, orig)
...
func TestViewMintRaw_AliasesBuffer(t *testing.T) {
	...
	// Accessors read current buffer contents.
	if view.SupplyU64() != 100 {
		t.Errorf("SupplyU64 = %d, want 100", view.SupplyU64())
	}
	// Mutate wire[] directly; view observes the change.
	wire[36] = 0xff // low byte of supply
...
func TestViewAccountRaw_AccessorsMatchFields(t *testing.T) {
	...
	if view.AccountState() != Initialized {
		t.Errorf("AccountState = %v, want Initialized", view.AccountState())
	}
	if view.HasDelegate() {
...
func TestDecodeAccountRaw_ShortBuffer(t *testing.T) {
	var a AccountRaw
	err := DecodeAccountRaw(&a, make([]byte, TokenAccountSize-1))
	if err == nil {
		t.Fatal("expected error on short account buffer")
	}
	// Our helper wraps binary.UnmarshalPOD which returns io.ErrShortBuffer.
...
// TestMarshalPOD_RoundTripAccount confirms bin.MarshalPOD + bin.UnmarshalPOD
// round-trip an AccountRaw to byte-identical wire output.
func TestMarshalPOD_RoundTripAccount(t *testing.T) {
	...
	// Decode via POD path into raw.
	var raw AccountRaw
	if err := DecodeAccountRaw(&raw, wire); err != nil {
		t.Fatal(err)
	}

	// Re-encode via POD path.
	...
	// Byte-for-byte identical to the original wire.
