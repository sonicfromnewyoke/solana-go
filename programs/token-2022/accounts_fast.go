...
// Token-2022 base account layouts are byte-identical to SPL-Token
// (Token-2022 extends via TLV after the base image). A Mint or Account
// wire buffer can be safely reinterpreted as MintRaw / AccountRaw as
// long as only the leading MintAccountSize / TokenAccountSize bytes are
// consulted — trailing extension data is ignored by the POD path.
const (
	MintAccountSize     = 82
...
// MintRaw is the POD-safe wire layout of a Token-2022 Mint account
// (82 bytes, identical to SPL-Token). See programs/token/accounts_fast.go
...
// AccountRaw is the POD-safe wire layout of a Token-2022 Account
// (165 bytes, identical to SPL-Token).
...
// MultisigRaw is the POD-safe wire layout of a Token-2022 Multisig
...
// longer than MintAccountSize — trailing extension (TLV) bytes are
// ignored, which is exactly the Token-2022 behavior we want.
func ViewMintRaw(data []byte) (*MintRaw, error) {
	if len(data) < MintAccountSize {
		return nil, fmt.Errorf("token2022: mint data too short: have %d, need %d", len(data), MintAccountSize)
	}
...
func ViewAccountRaw(data []byte) (*AccountRaw, error) {
	if len(data) < TokenAccountSize {
		return nil, fmt.Errorf("token2022: account data too short: have %d, need %d", len(data), TokenAccountSize)
	}
...
func ViewMultisigRaw(data []byte) (*MultisigRaw, error) {
	if len(data) < MultisigAccountSize {
		return nil, fmt.Errorf("token2022: multisig data too short: have %d, need %d", len(data), MultisigAccountSize)
	}
...
// Trailing extension (TLV) bytes past MintAccountSize are ignored.
...
// DecodeMintFast decodes a Token-2022 Mint account into the pointered
// Mint struct via the POD fast path. Drop-in replacement for the
// traditional `NewBinDecoder(data).Decode(&mint)` flow.
//
// One memcpy (82 bytes) + up to two *PublicKey allocations for present
// COption fields. Accepts buffers that include trailing extension (TLV)
// bytes — those are ignored by this path. If you need to parse the
// extensions, use the extension decoders separately.
...
// DecodeAccountFast decodes a Token-2022 Account via the POD fast
// path. Same trailing-TLV behavior as DecodeMintFast.
