...
func (acct *Account) UnmarshalWithDecoder(dec *bin.Decoder) (err error) {
	{
		v, err := dec.ReadNBytes(32)
		if err != nil {
			return err
		}
		acct.Mint = solana.PublicKeyFromBytes(v)
	}
	{
		v, err := dec.ReadNBytes(32)
		if err != nil {
		...
		acct.Owner = solana.PublicKeyFromBytes(v)
	}
	{
		v, err := dec.ReadUint64(binary.LittleEndian)
		if err != nil {
	...
	}
	{
		...
		if v == 1 {
			...
			acct.Delegate = solana.PublicKeyFromBytes(v).ToPointer()
		} else {
			_, err := dec.ReadNBytes(32)
			if err != nil {
				return err
	...
	}
	{
		...
		if v == 1 {
			...
			acct.CloseAuthority = solana.PublicKeyFromBytes(v).ToPointer()
		} else {
			_, err := dec.ReadNBytes(32)
			if err != nil {
				return err
...
func (acct Account) MarshalWithEncoder(encoder *bin.Encoder) (err error) {
	{
		err = encoder.WriteBytes(acct.Mint[:], false)
		if err != nil {
			return err
		}
	}
	{
		err = encoder.WriteBytes(acct.Owner[:], false)
		if err != nil {
			return err
		}
	}
	{
		err = encoder.WriteUint64(acct.Amount, bin.LE)
		if err != nil {
			return err
		}
	}
	{
		if acct.Delegate == nil {
			...
			err = encoder.WriteBytes(acct.Delegate[:], false)
			if err != nil {
				return err
			}
		}
	}
	err = encoder.WriteUint8(uint8(acct.State))
	if err != nil {
		return err
	}
	{
		if acct.IsNative == nil {
			err = encoder.WriteUint32(0, binary.LittleEndian)
			if err != nil {
				return err
			}
			...
			err = encoder.WriteUint64(*acct.IsNative, bin.LE)
			if err != nil {
				return err
			}
		}
	...
	{
		err = encoder.WriteUint64(acct.DelegatedAmount, bin.LE)
		if err != nil {
			return err
		}
	}
	{
		if acct.CloseAuthority == nil {
			err = encoder.WriteUint32(0, binary.LittleEndian)
			if err != nil {
				return err
			}
			...
			err = encoder.WriteBytes(acct.CloseAuthority[:], false)
			if err != nil {
				return err
			}
		}
