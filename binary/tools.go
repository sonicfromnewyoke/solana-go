...
func (rec *WriteByWrite) Write(b []byte) (int, error) {
	// Copy defensively: io.Writer's contract forbids retaining p, and the
	// Encoder reuses a per-call scratch buffer across primitive writes.
	cp := make([]byte, len(b))
	copy(cp, b)
	rec.writes = append(rec.writes, cp)
	return len(b), nil
}
